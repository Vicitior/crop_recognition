...
用法:
    # 使用所有模型
    python scripts/ensemble_predict_v2.py --data-dir dataset

    # 指定模型目录
    python scripts/ensemble_predict_v2.py --data-dir dataset --model-dirs saved_models/clip/clip-vit-large-patch14-336-v2 saved_models/clip/clip-vit-large-lora

    # 使用不同的集成策略
    python scripts/ensemble_predict_v2.py --strategy weighted
"""
...
def evaluate_ensemble(all_probs_list, labels, class_names, weights=None, strategy="average"):
    """评估集成效果"""
    if strategy == "average":
        # 简单平均
        if weights is None:
        ...
        ensemble_probs /= len(all_probs_list)
        ...
        for probs in all_probs_list:
            ...
            ensemble_probs += probs * rank_weights
        ensemble_probs /= len(all_probs_list)
...
def main():
    parser = argparse.ArgumentParser(description="模型集成评估 V2")
    ...
    parser.add_argument("--strategy", choices=["average", "weighted", "confidence", "rank"],
                        default="weighted", help="集成策略")
    ...
    strategies = ["average", "weighted", "confidence", "rank"]
    best_acc = 0
    best_strategy = None
    ...
    for strategy in strategies:
        ...
        acc, preds, class_acc = evaluate_ensemble(all_probs, labels, class_names, weights, strategy)

        print(f"\n[{strategy}] 集成准确率: {acc:.2f}%")
    ...
    # 显示最佳策略的详细结果
    print(f"\n{'='*60}")
    print(f"最佳策略: {best_strategy} ({best_acc:.2f}%)")
    print(f"{'='*60}")

    # 使用最佳策略重新计算
    val_accs = [c.get("best_val_acc", 50) for c in configs]
    weights = [a / sum(val_accs) for a in val_accs]
    acc, preds, class_acc = evaluate_ensemble(all_probs, labels, class_names, weights, best_strategy)

    print(f"\n每类准确率:")
    for cls, a in sorted(class_acc.items()):
        print(f"  {cls}: {a:.2f}%")
    ...
    improvement = best_acc - best_single
    print(f"\n{'='*60}")
    print(f"最佳单模型: {best_single:.2f}%")
    print(f"最佳集成:   {best_acc:.2f}%")
    ...
    results_path = Path("saved_models/clip/ensemble_results_v2.json")
